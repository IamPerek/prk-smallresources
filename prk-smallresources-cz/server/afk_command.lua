...
RegisterCommand('afk', function(src, args)
    ...
    if args[1] ~= '' then
        if argument then
            ...
            TriggerClientEvent('afk:cringe', -1, 'add', {sId = src, text = '~b~Jsem AFK~w~\n~b~Od doby:~w~ '..time.hour..':'..time.min..'\n~b~Duvod:~w~ '..argument})
        end
    end
end)
...
RegisterCommand('afkid', function(src, args)
    ...
    if args[1] ~= '' then
        if argument then
            ...
            TriggerClientEvent('afk:cringe', -1, 'add', {sId = plyId, text = '~b~Jsem AFK~w~\n~b~Od doby:~w~ '..time.hour..':'..time.min..'\n~b~Duvod:~w~ '..argument})
        end
    end
end, false)
