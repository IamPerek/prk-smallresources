...
local takeHostage = {
	allowedWeapons = { -- povolené zbraně
		`WEAPON_PISTOL`,
		`WEAPON_COMBATPISTOL`,
	},
	InProgress = false,
...
RegisterCommand("takehostage",function() -- 1. command
	callTakeHostage()
end)

RegisterCommand("th",function() -- 2. command
	callTakeHostage()
end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Potrebujete pistoli s municí, abyste zajali rukojmí namírenou zbraní!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~Nikdo není poblíž koho by bylo možné vzít jako rukojmí!")
			end
		else
			drawNativeNotification("~r~Nikdo není poblíž koho by bylo možné vzít jako rukojmí!")
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Stiskni [G] uvolnit, [H] zabít")
